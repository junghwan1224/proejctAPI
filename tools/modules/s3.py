...
class S3:
    ...
    def upload(self, local_addr, remote_addr):
        remote_addr = remote_addr[0:] if remote_addr.startswith(
            '/') else remote_addr
        extension = local_addr.split('/')[-1].split('.')[-1].lower()
        return self._conn.upload_file(local_addr, self._bucket_name,
                                      remote_addr, ExtraArgs={
                                          "ContentType": get_mimeType(extension)
                                      })
...
def get_mimeType(extension):
    ...
    if extension not in mimeTypes.keys():
        return ""
